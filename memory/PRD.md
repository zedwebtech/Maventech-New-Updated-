...
- Production host (`Host: maventechsoftware.com`):
  ...
  - The stale `company_logo` DB row pointing at
    `https://romantic-ardinghelli-9.preview.emergentagent.com/uploads/...` is
...
- Follow-up fix (alignment bug): variable title/teaser line counts (1 vs 2 lines) were shifting the In-stock pill/meta/price across cards in a row. Gave .product-title (line-height 1.3, min-height 2.6em) and .pc-teaser (min-height 2.8em) a fixed 2-line reserve so every grid card aligns. Verified: price-row tops identical (445px) across the first row.
- Follow-up fix (card buttons clipped + ugly "Added" state): the price + Add + Buy were crammed on one line and overflowed the narrow card (Buy clipped by card overflow:hidden), worse in the wider "Added" state. Restructured render_product_card so the price is its own row and the Add/Buy buttons sit on a new full-width .pc-btn-row split 50/50 (flex:1) — nothing clips in any state. Recolored the card "Added" state (body:not(.adm) .pc-btn-cart.added) to a calm green success pill instead of a second heavy blue, so it reads as confirmation beside the blue Buy. Verified via screenshot: both buttons inside card bounds, aligned across all cards.

## 2026-06-29 — GSC fixes: /hub/ 404s, broken canonical, login meta description
- Root cause (404s + broken canonical, ~250 URLs): topic hubs render at the virtual clean URL /hub/<slug>, but the header's CSS/JS and the nav/footer/body links are RELATIVE, so the browser/Googlebot resolved them against /hub/ → bogus URLs (/hub/shop.php, /hub/category.php?slug=…, /hub/page.php?…, /hub/hub/<slug>, plus /au/hub/… /ca/hub/…) that 404 on production. On Apache these also broke CSS loading (no /hub/assets rule) → unstyled hubs. And hub.php never set $canonicalUrl, so the header built it as /hub.php (no ?topic) which 404s = "broken canonical".
- Fix 1 (stop generation): hub.php now sets $baseHref = site_url().country_prefix().'/'; header.php emits <base href> (right after <head>, before the first relative <link>) only when $baseHref is set. Region-aware so /au/hub/... resolves relative links to /au/. Fixes assets, nav, footer, hub-body links and cart AJAX in one shot on both preview + production. Verified: hub page fully styled, 0 hub/*.php links in markup.
- Fix 2 (canonical): hub.php sets $canonicalPathBare='/hub/<slug>' + $canonicalUrl=site_url().country_prefix().that → canonical is now the clean valid /hub/<slug> (was broken /hub.php). Drives correct hreflang too.
- Fix 3 (cleanup already-indexed bad URLs): .htaccess + router.php 301-redirect ^hub/hub/(.+)→/hub/$1, ^hub/(.+\.php)→/$1 (excluding assets|ajax|uploads), placed before the clean /hub/<slug> rule. Verified: /hub/shop.php→301 /shop.php, /hub/category.php?slug=antivirus→301 (query preserved), /hub/hub/windows→301 /hub/windows, /hub/antivirus still 200.
- Fix 4 (meta description missing): login.php is a standalone page with its own <head> (not the shared header) — added <meta name="description"> there. Verified present on /login.php and /login.php?next=…
- ENV: pod reset this session — reinstalled PHP 8.2 + MariaDB, started mysqld_safe, restarted frontend (start.sh reseeded). Deploy to maventechsoftware.com + Validate Fix in GSC to clear the three issue categories.


## 2026-06-29 — Public "Leave a review" flow (clears GSC aggregateRating/review warnings the safe way)
- Context: GSC "Missing field aggregateRating" + "Missing field review" (Product snippets, non-critical) appeared because 0/37 products have any published reviews, so product.php emits no review markup. User chose the safe path (NO fake reviews): add a public review-collection flow so genuine reviews accumulate and stars appear naturally.
- New endpoint ajax/product-review.php: verified-purchase gated — requires order_number + email that match a PAID order containing that product; one review per order+product; auto-publishes 3★+ (status=published), hides <3★ for admin follow-up (same policy as review.php / success-review.php). Pings admin bell. Nothing is fabricated.
- product.php reviews section reworked: always shows a "Write a review" CTA (data-testid write-review-btn) + a Bootstrap modal (#writeReviewModal: star picker, comment, order#, email; posts JSON to ajax/product-review.php, reloads on publish). When a product has reviews it shows the aggregate + cards; when none it shows a "be the first to review" empty state (data-testid reviews-empty). JSON-LD aggregateRating/review remains conditional on real published reviews (unchanged) so no fake markup is ever emitted.
- Verified via curl: bad order/email → reject, valid order but wrong product → reject, valid verified purchase 5★ → published + aggregateRating JSON-LD (reviewCount:1, ratingValue:5.0) + on-page "Based on 1 verified review", duplicate → blocked. Test review deleted afterward to keep data clean. Screenshot confirms modal UI.
- As real customers review purchases (incl. the 1 already pending in admin → approve it), each product gains aggregateRating/review markup and the GSC warnings clear on their own. Existing post-purchase asks (order-success success-review.php + email token review.php) still feed the same pipeline.

## 2026-06-29 — Hub "broken canonical URLs" fully resolved (follow-up to the /hub/ fix)
- GSC still listed hub URLs (/hub/<topic>, /au/hub/<topic>, /ca/hub/<topic>, and bare /hub.php, /au/hub.php, /ca/hub.php) under "Pages containing broken canonical URLs". Two distinct causes:
  1) Valid hub pages (200) previously emitted canonical → /hub.php (header default when hub.php set no $canonicalUrl), and /hub.php 404s → broken canonical. ALREADY fixed earlier this day by setting $canonicalUrl/$canonicalPathBare to the clean /hub/<slug>. Re-verified: /au/hub/microsoft-office → 200, robots index,follow, canonical = self (.../au/hub/microsoft-office).
  2) Bare /hub.php and any invalid topic are 404s but were still robots index,follow with a self/-/hub.php canonical (a 404 used as canonical = broken). FIX (hub.php 404 branch): set $noIndex=true and $canonicalUrl = site_url().country_prefix().'/shop.php' so the 404 is noindex with a valid canonical. Verified: /hub.php → 404 + noindex + canonical /shop.php; /au/hub.php → 404 noindex canonical /au/shop.php.
- Also removed the ugly param-form internal links that created duplicate /hub.php?topic= URLs: related-hub cards (hub.php line ~367), the "Topics" breadcrumb (line ~193), and the 404-branch topic buttons (line ~50) now use clean base-href-relative hub/<slug> URLs → resolve to /hub/<slug> (or /<cc>/hub/<slug>). Verified related links render as href="hub/<slug>".
- Note: many topics in GSC (office-2024-pc, windows-10, office-2019-mac, …) are admin-created hubs valid (200) on production; locally only the 3 defaults exist so they 404 (now safely noindex). Sitemap already emits only valid topic_hubs_all(true) slugs. ALL changes are server-side — DEPLOY to maventechsoftware.com then GSC → Validate Fix on "broken canonical URLs".

## 2026-06-29 — Hub HTTP 4xx cleanup completed (final pass)
- GSC "HTTP 400-499" listed ~110 URLs, almost all the /hub/<file> relative-link mis-resolutions (/hub/category.php?slug=, /hub/product.php?slug=, /hub/page.php?slug=, /hub/reviews.php, /hub/account.php, /hub/cart.php, /hub/shop.php, /hub/brand.php?slug=, /hub/blog-post.php?id=, /hub/hub.php?topic=, /hub/hub/<slug>, + /au/ /ca/ variants), plus bare /hub.php and two root product slugs.
- These are pre-deploy crawl artifacts: the base href + the .htaccess/router 301s (^hub/hub/(.+)→/hub/$1, ^hub/(.+\.php)→/$1 excl assets|ajax|uploads) already cover them. Re-verified the full matrix on the router: every /hub/<file>.php → 301 to /<file> (query preserved), /hub/hub.php?topic=x → /hub.php?topic=x, /hub/hub/windows → /hub/windows, country-prefixed /au//ca/ variants → 301 too.
- New this pass:
  * hub.php no-topic branch: bare /hub.php (topicSlug==='') now 301s to (country_prefix).'/shop.php' (covers Apache + router in one place); an INVALID topic still 404s noindex. Verified /hub.php→/shop.php, /au/hub.php→/au/shop.php, /hub/office-2099→404.
  * 404.php bare-slug rescue: before serving 404, if the single path segment (region prefix stripped) matches an active product/category/brand slug → 301 to product.php/category.php/brand.php. Verified /windows-10-home and /microsoft-office-2019-home-business-pc → 301 /product.php?slug=…; bogus slug stays 404. Runs on both ErrorDocument 404 (Apache) and router fallback.
- STILL REQUIRES DEPLOY to maventechsoftware.com — GSC reports reflect the last crawl of the OLD code. After deploy: GSC → Validate Fix on the "HTTP 400-499" issue. The 301s consolidate link equity; base href stops new /hub/ URLs being generated.

## 2026-07-13 — FOUC on navigation + mobile footer registration-number overlap
- Bug 1 (page flicker on navigation, live domain): root cause was a background-color MISMATCH between the inline critical CSS (`#050B1B`) and the deferred `dark-mode-polish.css` (`#131525` with radial gradients). Because `dark-mode-polish.css` was loaded async via `<link rel="preload" as="style" onload="…">`, dark-mode users saw a visible "near-black → dark navy" repaint on every page navigation.
- Fix 1a (`includes/header.php`): updated the `#critical-theme-pre-paint` inline CSS to use `#131525` for `[data-bs-theme="dark"]` so the very first paint matches the polish stylesheet's background exactly (zero repaint delta).
- Fix 1b (`includes/header.php`): switched `dark-mode-polish.css` from async preload to a synchronous `<link rel="stylesheet">`. The file is small and only contains `[data-bs-theme="dark"]` rules — light-mode users see zero impact; dark-mode users get the polished bg + inputs + focus rings on the very first paint. Eliminates the flicker deterministically.
- Bug 2 (footer "Company Registration Number" mobile overlap): the copyright + reg-number line was a single `<div>` joined by a `·` middle dot, which produced awkward wrapping on 375-414px screens.
- Fix 2 (`includes/footer.php` + `assets/css/style.css`): split the line into `.footer-copyright-main`, `.footer-copyright-sep`, `.footer-copyright-reg` spans. On tablet+desktop the inline `·`-separated layout is preserved; below 576px the separator is hidden and the reg-number stacks below the copyright as its own line (`.78rem` muted). Also gave the top-of-footer contact-list reg-number pill `word-break: break-word` for iPhone SE (<376px).
- Verified via mobile (390×844) + desktop (1440×900) screenshots at localhost:3000 after reinstalling PHP 8.2 + MariaDB in the fresh forked pod. Both viewports render as intended; PHP `-l` reports no syntax errors on the two modified includes; `style.min.css` auto-regenerates from the updated source.
- No third-party integrations touched. No JS behaviour changed. DEPLOY to maventechsoftware.com to see the fix live.

## 2026-07-13 — Google Merchant Center "Internal error (9, 0)" on 2 product rows
- Symptom: GMC feed upload report showed two rows failing with "Internal error (9, 0)" at lines 1,269 and 1,619 (Item Id EMPTY — meaning Google's parser choked BEFORE it could even read the item's `<g:id>`). 2 products affected, error persisted across resubmits.
- Investigation: reproduced locally at localhost:3000/merchant-feed.php. Lines 1269 + 1619 fall inside `<g:product_detail>` blocks and BOTH blocks emit `<g:attribute_value>Digital download — emailed product key</g:attribute_value>` — the only cross-item text with a typographic em-dash (U+2014). Combined with the empty Item Id + persistent line numbers (not transient), the diagnosis is a strict-XML/GMC-validator issue with control chars or typographic punctuation in narrow attributes.
- Fix (`merchant-feed.php`, two edits):
  1. Hardened `feed_xml_esc()` to STRIP characters illegal in XML 1.0 (`\x00-\x08\x0B\x0C\x0E-\x1F\x7F` — PHP's `htmlspecialchars` lets these through and any strict XML parser must reject them). Added a `strtr()` normaliser for em-dash / en-dash / smart quotes / ellipsis / NBSP → ASCII equivalents (same visual meaning, zero risk in GMC/Bing/Meta). Belt: single pinch-point, no future emit-site can forget.
