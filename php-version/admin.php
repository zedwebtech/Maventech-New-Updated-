...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_product') {
        ...
        // Promo bar customization — discount percent + coupon code.
        $__pbPct = (int) ($_POST['promo_bar_percent'] ?? 10);
        if ($__pbPct < 1)  $__pbPct = 1;
        if ($__pbPct > 95) $__pbPct = 95;
        setting_set('promo_bar_percent', (string) $__pbPct);
        $__pbCode = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', (string) ($_POST['promo_bar_code'] ?? 'MAVEN10')));
        setting_set('promo_bar_code', $__pbCode !== '' ? substr($__pbCode, 0, 24) : 'MAVEN10');
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f1f5f9;padding:32px 0;">
  <tr><td align="center">
    <table width="580" cellpadding="0" cellspacing="0" border="0" style="max-width:580px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 24px rgba(15,23,42,.10);">
      ...
      <tr><td style="background:#0f172a;padding:20px 32px;color:#94a3b8;font-size:11.5px;line-height:1.55;text-align:center;">
    ...
    } elseif ($action === 'google_review_add') {
        // Add a manually-transcribed Google Business Profile review so it
        // renders on the public /reviews.php page with a "From Google" badge.
        $name       = trim((string)($_POST['name'] ?? ''));
        ...
        if ($name !== '' && $text !== '') {
            $initials = strtoupper(mb_substr(preg_replace('/[^A-Za-z]/', '', $name), 0, 2)) ?: 'G';
            $pdo->prepare('INSERT INTO reviews (name, initials, location, review_date, rating, text, product, verified, source, source_url, avatar_url) VALUES (?,?,?,?,?,?,?,1,?,?,?)')
                ->execute([$name, $initials, $location, $reviewDate, $rating, $text, $product, 'google', $sourceUrl, $avatarUrl]);
        }
        header('Location: admin.php?tab=reviews&sub=google&msg=Google+review+added'); exit;
    } elseif ($action === 'google_review_delete') {
        $pdo->prepare("DELETE FROM reviews WHERE id=? AND source='google'")->execute([(int)$_POST['review_id']]);
        header('Location: admin.php?tab=reviews&sub=google&msg=Google+review+deleted'); exit;
    } elseif ($action === 'save_google_profile_url') {
        setting_set('google_reviews_profile_url', trim((string)($_POST['profile_url'] ?? '')));
        header('Location: admin.php?tab=reviews&sub=google&msg=Google+profile+URL+saved'); exit;
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
  <div>
    <h4 class="fw-bold mb-1" data-testid="subscription-title"><i class="bi bi-shield-shaded text-primary me-2"></i>Protection Hub</h4>
    ...
    <form id="ciEdit" method="post" class="d-none mt-3" data-testid="ci-edit-form">
      ...
      <div class="row g-3">
        ...
        <div class="col-12">
          ...
          <?php $promoOn = (setting_get('promo_bar_enabled', '0') === '1'); $promoPct = (int) setting_get('promo_bar_percent', '10'); $promoCode = setting_get('promo_bar_code', 'MAVEN10'); ?>
          <div class="p-3 mt-3" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;">
            ...
            <div class="row g-2 mt-2">
              <div class="col-sm-4">
                ...
                <div class="input-group input-group-sm">
                  <input class="form-control" type="number" min="1" max="95" name="promo_bar_percent"
                         value="<?= (int)$promoPct ?>" oninput="var e=document.getElementById('ciPromoPctPreview'); if(e)e.textContent=this.value;"
                         data-testid="ci-promo-percent-input">
                  <span class="input-group-text">%</span>
                </div>
              </div>
              <div class="col-sm-8">
                <label class="form-label small fw-semibold mb-1">Promo code</label>
                <input class="form-control form-control-sm text-uppercase" type="text" maxlength="24" name="promo_bar_code"
                       value="<?= esc($promoCode) ?>" placeholder="MAVEN10"
                       oninput="this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,''); var e=document.getElementById('ciPromoCodePreview'); if(e)e.textContent=this.value;"
                       data-testid="ci-promo-code-input">
              </div>
            </div>
          </div>
        </div>
