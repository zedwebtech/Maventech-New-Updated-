 ...
 */
/**
 * The best public base URL (scheme + host, no trailing slash) to use when
 * building ABSOLUTE links/images for an email or PDF. Prefers the admin-
 * configured production domain, skips stale Emergent preview / cluster /
 * localhost values (common right after a DB import), and falls back to the
 * live request host. Always returns a host-qualified base so nothing in an
 * email ever renders as a broken root-relative path — including when the
 * message is generated from a CLI/cron job with no Host header.
 */
function email_public_base(): string {
    $isBadHost = static function (string $h): bool {
        $h = strtolower($h);
        return $h === ''
            || (bool)preg_match('/(?:^|\.)preview\.emergentagent\.com$/i', $h)
            || (bool)preg_match('/\.preview\.emergentcf\.cloud$/i', $h)
            || in_array($h, ['localhost', '127.0.0.1', '0.0.0.0'], true);
    };
    // 1) A genuine, non-preview production domain wins.
    foreach ([(string)setting_get('site_domain_url', ''), (string)setting_get('main_url', ''), site_url()] as $cand) {
        $cand = trim($cand);
        if ($cand === '') continue;
        if ($isBadHost((string)parse_url($cand, PHP_URL_HOST))) continue;
        return rtrim($cand, '/');
    }
    // 2) Fallback: any host-qualified value (incl. RAW stored preview URL, which
    //    a normal read may have stripped to a path) — anything but localhost.
    foreach ([
        (string)setting_get('site_domain_url', '', true),
        (string)setting_get('main_url', '', true),
        (string)setting_get('site_domain_url', ''),
        (string)setting_get('main_url', ''),
        site_url(),
    ] as $cand) {
        $cand = trim($cand);
        $h = strtolower((string)parse_url($cand, PHP_URL_HOST));
        if ($h === '' || in_array($h, ['localhost', '127.0.0.1', '0.0.0.0'], true)) continue;
        return rtrim($cand, '/');
    }
...
function email_absolute_url(string $url): string {
    ...
    return email_public_base() . '/' . ltrim($url, '/');
}
...
function render_template(string $code, array $vars = []): ?string {
    ...
    if ($trackOrder !== null && strpos($html, '{{track_order_button}}') === false) {
        $out = inject_track_order_cta($out, $trackOrder, email_public_base());
    }
    return $out;
}
