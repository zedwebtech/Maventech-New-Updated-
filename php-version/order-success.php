...
<?php endif; ?>
    ...
    <div class="col-12 col-md-4 order-md-1">
      ...
      <?php
        /* Standalone "Loved your purchase? — Leave us a Google review" card
           REMOVED per user request (2026-07-07).  Two Google-review CTAs
           on the same success page were confusing shoppers.  The single
           surviving Google share button lives INSIDE the on-site review
           thank-you state ("Mind sharing it on Google too? — Post my
           review on Google"): it only appears AFTER the buyer has left
           an on-site review, funneling that same review text via
           navigator.clipboard so they can paste it on Google in one
           click.  That contextual, post-review CTA is a higher-signal
           entry point than the standalone always-visible card, so
           removing the standalone card cleans the page + eliminates the
           duplicate CTA the user flagged.

           NOTE: the surviving Google-share pill in srThanks uses
           `class="gr-btn"` — we keep the .gr-btn CSS rules below so
           that button still styles correctly. The .gr-card / .gr-title
           / .gr-stars / .gr-foot rules were dropped along with the
           removed card HTML. */
      ?>
      <style>
        /* Kept — used by the "Post my review on Google" button inside the srThanks state. */
